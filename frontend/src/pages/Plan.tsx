...
import {
  ...
  Waves, Zap, UserCog, ChevronDown, Trash2, AlertTriangle, TrendingUp,
} from 'lucide-react'
          ...
          <div className="bg-bg-secondary rounded-xl border border-white/5 p-5 space-y-4">
            ...
            {generate.isError && (
              <p className="text-xs text-danger">
                Failed to generate plan. Check AI settings.
              </p>
            )}
          </div>
        </div>
        ...
        <>
          ...
          <div className="space-y-2">
            ...
            {plan.plan.progression_assessment?.peak_hours && (
              <div className="flex items-center gap-2 text-xs text-slate-500 bg-bg-secondary rounded-xl border border-white/5 px-4 py-2.5">
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      {w.steps.map((s: any, si: number) => (
                                        <div key={si} className="flex items-center gap-2 text-xs text-slate-500">
                                          <span className="w-16 text-right text-slate-600">{Math.floor(s.duration / 60)}:{(s.duration % 60).toString().padStart(2, '0')}</span>
                                          ...
                                          {s.power && <span className="text-slate-400">{Math.round(s.power * 100)}% FTP</span>}
                                          {s.repeat && <span className="text-accent">{'×'}{s.repeat}</span>}
                                          {s.notes && <span className="text-slate-600 italic">— {s.notes}</span>}
                                        </div>
                                      ))}
                                    </div>
                                  )}
